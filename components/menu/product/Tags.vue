...
<template>
    <div class="tag-input relative">
        ...
        <div v-if="duplicate" class="text-sm mt-2 text-brand-error">
            This tag already exists
        </div>

        <div v-if="tags.length" class="flex gap-2 overflow-x-auto mt-2 pb-3">
            <span v-for="(tag, index) in tags" :key="tag"
                class="flex items-center gap-2 pl-3 pr-2 py-2 bg-brand-green-400 bg-opacity-10 rounded-lg flex-shrink-0 cursor-pointer"
