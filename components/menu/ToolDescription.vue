<script setup>

</script>
<template>
    <div class="my-10">
        <h3 class="text-xl font-bold mb-6">How it works in just 3 simple steps:</h3>
        <div>
            <div>
                <span class="font-semibold">1. Create - </span>
                <span>categories like starters, mains, drinks, and seasonal specials, fill in product details, and add appetizing food images to attract customers and encourage more orders.</span>
            </div>
            <div class="mt-6">
                <span class="font-semibold">2. Copy - </span>
                <span>Browse our collection of eye-catching, pre-designed menu templates and select one. The designs are fast, modern and responsive.</span>
            </div>
            <div class="mt-6">
                <span class="font-semibold">2. Paste - </span>
                <span>Iclude it directly into your preferred project management tool, website or app. Your beautiful, digital menu is ready to impress customers and drive sales.</span>
            </div>
        </div>
    </div>
    <div class="my-10">
        <h3 class="text-xl font-bold mb-6">What is a digital food menu ?</h3>
        <div>A digital food menu is an electronic version of a restaurant's menu displayed on a screen or accessed through a website or mobile app. Unlike a static PDF, it's dynamic and interactive, offering several advantages over traditional paper menus. </div>
    </div>
    <div class="my-10">
            <h3 class="text-xl font-bold mb-6">Digital menu advantages:</h3>
        <ul>
            <li>
                1. Convenient browsing: Customers can quickly explore menus on their devices, bypassing physical menus and wait times. Ordering is simple with a few taps.
            </li>
            <li>
                2. On-demand adaptability: Easily update prices, specials, or entire menus without having to reprint them. Provide seasonal delights to keep things interesting and exciting.
            </li>
            <li>
                3. Delightful visuals: Impress customers with high-quality photos and engaging descriptions. Showcase your culinary creations and increase order value.
            </li>
            <li>
                4. Save the environment by going digital: Eliminate printing costs and reduce environmental impact. Digital menus are both cost-effective and environmentally friendly, which benefits everyone.
            </li>
            <li>
                5. Enhance the experience: Provide customers with interactive features such as nutritional information, allergen flags, and even custom orders. Increase trust and satisfaction through transparency and personalization.
            </li>
            <li>
                6. SEO boost for your business: Attract more diners by improving your website's visibility with keyword-rich digital menus that search engines love.
            </li>
        </ul>
    </div>
</template>