        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div class="lg:col-span-2">
                <div class="flex-col gap-1 max-w-sm m-auto flex md:hidden mt-2 mb-6">
                    ...
                    <input type="url" name="" v-model="qrConfigs.data" class="border rounded p-2 border-gray-300" placeholder="https://" />
                </div>
                            ...
                            <div class="bg-slate-100 p-4 rounded">
                                <div class="flex flex-wrap gap-6">
                                    <div @click="qrConfigs.dotsOptions.type = 'square'"
                                        class="h-12 w-12 border border-gray-200 rounded cursor-pointer bg-white p-1.5" title="Square">
                                        <QrCodeIconsSquareDots />
                                    </div>
                                    <div @click="qrConfigs.dotsOptions.type = 'classy'"
                                        class="h-12 w-12 border border-gray-200 rounded cursor-pointer bg-white p-1.5" title="Classy">
                            ...
                            <div class="bg-slate-100 p-4 rounded">
                                <div class="flex flex-wrap gap-6">
                                    <div @click="qrConfigs.margin = nr - 1"
                                        class="h-12 w-12 border border-gray-200 rounded cursor-pointer bg-white p-1.5 flex items-center justify-center font-bold"
                                        v-for="nr in 4"> {{ nr - 1 }}
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="bg-slate-100 p-4 rounded">
                                <div class="flex flex-wrap gap-6">
                                    <div @click="qrConfigs.cornersDotOptions.type = ''"
                                        class="h-12 w-12 border border-gray-200 rounded cursor-pointer bg-white p-1.5 grid place-content-center" title="Default">
                                         <UIcon name="i-ph-x-light" class="text-red-600 text-2xl" />
                                    </div>
                            ...
                            <div class="bg-slate-100 p-4 rounded">
                                <div class="flex gap-6 items-center">
                                    <div v-if="fileLogoName" @click="clearLogo"
                                        class="h-12 w-12 border border-gray-200 rounded cursor-pointer bg-white flex items-center shrink-0 justify-center">
                                        <!-- <IconsClose class="text-red-600" /> -->
                                        <UIcon name="i-ph-x-light" class="text-red-600 text-2xl" />
                                    </div>
                                    <label v-else
                                        class="h-12 w-12 border border-gray-200 shrink-0 border-info-700 rounded cursor-pointer bg-white flex items-center justify-center">
                        ...
                        <div>
                            ...
                            <div class="bg-slate-100 p-4 rounded">
                                <div class="flex gap-6 justify-between">
                                    <div class="flex items-center rounded overflow-hidden border border-gray-200">
                                        <div @click="handleLogoSize('up')"
                                            class="p-1 w-12 h-12 bg-white cursor-pointer flex items-center justify-center">
                                            <UIcon name="i-ph-plus-light" class="text-2xl"/>
                                        </div>
                                        <div class="p-2 h-12 w-12 flex items-center justify-center"> {{
            ...
            <div>
                <div class="flex-col gap-1 max-w-sm m-auto hidden md:flex">
                    <label class="font-medium text-lg shrink-0">Your URL</label>
                    <input type="url" name="" v-model="qrConfigs.data" class="border border-gray-300 rounded p-2" placeholder="https://" />
                </div>
                ...
                <div class="flex gap-4 mt-6 justify-center">
                    <UButton variant="soft" @click="downloadQr('png')">Save PNG</UButton>
                    <UButton variant="soft" @click="downloadQr('jpeg')">Save JPEG</UButton>
                    <UButton variant="soft" @click="downloadQr('svg')">Save SVG</UButton>
            </div>
            </div>
        </div>
    </UCard>
