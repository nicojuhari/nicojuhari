...
export default function Checklist() {
  ...
  const print = () => {
    ...
    w.document.write(`<!DOCTYPE html><html><head><title>${title}</title><style>body{font-family:Arial,sans-serif;max-width:600px;margin:20px auto}h1{border-bottom:2px solid #333;padding-bottom:10px}</style></head><body><h1>${title}</h1>${rows}</body></html>`)
    w.document.close()
    w.print()
  }
    ...
    <div className="space-y-6">
            ...
            {tasks.map((task) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {task.notes && (
                    <p className="text-xs text-muted-foreground line-clamp-1 mt-0.5">{task.notes}</p>
                  )}
                </div>
      ...
      {tasks.length > 0 && (
        <div className="flex flex-wrap gap-3 justify-between">
          <AlertDialog>
            <AlertDialogTrigger
              render={<Button variant="ghost" className="text-destructive hover:text-destructive gap-1.5" />}
            >
