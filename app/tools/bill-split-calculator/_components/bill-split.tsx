...
export default function BillSplit() {
  ...
  const addExpense = () => {
    ...
    if (!toSplitEqually && restFromSplit !== 0) {
      setExpenseError(`Split amounts must equal the total (${restFromSplit > 0 ? "missing" : "over by"} ${Math.abs(restFromSplit).toFixed(2)}).`)
      setTimeout(() => setExpenseError(""), 2500)
      return
    }
    ...
    if (toSplitEqually) {
      ...
      splitBetween = friends.map((f, i) => ({
        id: f.id,
        amount: i === friends.length - 1
          ? parseFloat((form.amount - eq * (friends.length - 1)).toFixed(2))
          : eq,
      }))
    } else {
  ...
  const { personPaid, personSpent, personBalance } = useMemo(() => {
    ...
    friends.forEach((f) => { paid[f.id] = 0; spent[f.id] = 0 })
    expenses.forEach((e) => {
      if (e.paidBy) paid[e.paidBy] = (paid[e.paidBy] ?? 0) + e.amount
      e.splitBetween.forEach((s) => { spent[s.id] = (spent[s.id] ?? 0) + s.amount })
    })
    const balance: Record<string, number> = {}
    friends.forEach((f) => { balance[f.id] = (paid[f.id] ?? 0) - (spent[f.id] ?? 0) })
        ...
        <div className="border rounded-xl p-4 bg-white space-y-3">
          ...
          {friends.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {friends.map((f) => (
                <span key={f.id} className="inline-flex items-center gap-1 border rounded-md pl-2.5 pr-1 py-1 text-sm bg-muted/40">
                  {f.name}
        ...
        <div className="border rounded-xl p-4 bg-white space-y-3">
          ...
          <label className="flex items-center gap-2 cursor-pointer text-sm">
            <input
              ...
              onChange={(e) => { setToSplitEqually(e.target.checked); setCustomSplits({}) }}
            ...
            <div className="space-y-2">
              ...
              {restFromSplit !== 0 && (
                <p className={`text-sm ${restFromSplit > 0 ? "text-amber-600" : "text-destructive"}`}>
                  {restFromSplit > 0 ? `Remaining: ${restFromSplit.toFixed(2)}` : `Over by: ${Math.abs(restFromSplit).toFixed(2)}`}
                </p>
              )}
            </div>
          )}
          ...
          <div className="flex justify-end">
            <Button onClick={addExpense} disabled={friends.length < 2}>Add Expense</Button>
          </div>
          {friends.length < 2 && (
            <p className="text-muted-foreground text-xs text-right">Add at least 2 people first.</p>
          )}
        </div>
      </div>
                      ...
                      <div className="flex flex-wrap gap-x-2 gap-y-0.5">
                        {e.splitBetween.map((s) => (
                          <span key={s.id} className="whitespace-nowrap">
                            {getName(s.id)} <span className="text-muted-foreground">({s.amount.toFixed(2)})</span>
                          </span>
                        ))}
                      </div>
                    </td>
      ...
      <div className="border rounded-xl bg-white overflow-hidden">
        ...
        <div className="p-4">
          {settlements.length > 0 ? (
            <div className="space-y-2">
              {settlements.map((s) => (
                <div key={`${s.from}-${s.to}`} className="flex items-center justify-between bg-muted/40 px-3 py-2.5 rounded-lg border">
                  ...
                  <div key={f.id} className="border rounded-lg p-4 min-w-[200px] shrink-0">
                    ...
                    <div className="space-y-1.5 text-sm">
                      ...
                      <div className="flex justify-between border-t pt-1.5 mt-1.5">
                        ...
                        <span className={`font-bold font-mono ${balance > 0.01 ? "text-green-600" : balance < -0.01 ? "text-red-500" : "text-muted-foreground"}`}>
                          {balance > 0.01 ? `+${balance.toFixed(2)}` : balance < -0.01 ? balance.toFixed(2) : "0.00"}
                        </span>
                      </div>
                    </div>
                  </div>
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>This will remove all friends and expenses. This cannot be undone.</AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogCancel render={<Button variant="outline" />}>Cancel</AlertDialogCancel>
                <AlertDialogAction render={<Button />} onClick={() => { setFriends([]); setExpenses([]) }}>Delete</AlertDialogAction>
              </AlertDialogFooter>
            </AlertDialogContent>
          </AlertDialog>
        </div>
