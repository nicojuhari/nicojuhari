...
<template>
    <h1 class="text-center title py-4 mt-12">Whitespace Remover</h1>
    <p class="mb-8 text-lg font-normal text-center max-w-xl mx-auto">Use the whitespace remover tool to replace text spaces with dashes (hyphens), underscores, slashes or new lines. Simple and fast.</p>
            ...
            <UCard>
                <div class="space-y-6">
                    ...
                    <URadioGroup v-model="transformText" legend="Transform the case?" :items="transformOptions" variant="table" size="sm" />
                     <USwitch label="Reverse the text?" v-model="reverseText" size="sm" />
                     <USwitch label="Keep the new lines?" v-model="keepNewlines" size="sm" />
                </div>
            </UCard>
        </div>
    </div>
